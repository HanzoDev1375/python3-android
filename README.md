...
2. You will need a separate build run for every API Level/architecture combination you wish to run on:
   ...
   * Build 64 bit `sudo docker run --rm -it -v $(pwd):/python3-android -v /mnt/c/SPE/android-ndk-r26b --env ARCH=arm64 --env ANDROID_API=26 python:3.12.1-slim /python3-android/docker-build.sh --enable-shared --without-ensurepip --disable-ipv6 --with-build-python`
   * Build 32 bit `sudo docker run --rm -it -v $(pwd):/python3-android -v /mnt/c/SPE/android-ndk-r26b --env ARCH=arm --env ANDROID_API=26 python:3.12.1-slim /python3-android/docker-build.sh --enable-shared --without-ensurepip --disable-ipv6 --with-build-python`
   * Build x86_64 `sudo docker run --rm -it -v $(pwd):/python3-android -v /mnt/c/SPE/android-ndk-r26b --env ARCH=x86_64 --env ANDROID_API=26 python:3.12.1-slim /python3-android/docker-build.sh --enable-shared --without-ensurepip --disable-ipv6 --with-build-python`
